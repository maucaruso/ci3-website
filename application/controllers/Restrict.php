...
class Restrict extends CI_Controller 
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('user_id')) {
			...
			$this->template->show('restrict/restrict.php', $data);
		} else {
			$data = array(
				"scripts" => array(
					"util.js",
			...
			$this->template->show('restrict/login.php', $data);
		}
	}
	...
	public function ajax_login()
	{
		...
		if (!$this->input->is_ajax_request()) {
			exit('Acesso direto de script não é permitido');
		}
	...
	public function ajax_import_image()
	{
		...
		if (!$this->upload->do_upload('image_file')) {
			...
			if ($this->upload->data()['file_size'] <= 1024) {
				...
				$json['status'] = 0;
				$json['error'] = 'O tamanho máximo de arquivos é 1MB';
			}
		}
	...
	public function ajax_save_course()
	{
		...
		if (empty($data['course_duration'])) {
			...
			if (!($data['course_duration'] > 0 && $data['course_duration'] < 100)) {
				$json['error_list']['#course_duration'] = 'A duração deve ser maior que zero e menor que 100 horas';
			}
		}
	...
	public function ajax_save_user()
	{
		...
		if (empty($data['user_email'])) {
			...
			if ($this->users_model->is_duplicated('user_email', $data['user_email'], $data['user_id'])) {
				$json['error_list']['#user_email'] = 'E-mail já cadastrado no sistema';
			} else {
		...
		if (empty($data['user_password'])) {
			$json['error_list']['#user_password'] = 'A senha do usuário é obrigatório';
		} else {
	...
	public function ajax_get_course_data()
	{
		...
		$json['img']['course_img_path'] = $data['course_img'];
	...
	public function ajax_get_member_data()
	{
		...
		$json['img']['member_photo_path'] = $data['member_photo'];
	...
	public function ajax_delete_course_data()
	{
		...
		$course_id = $this->input->post('course_id');
		$data = $this->courses_model->delete($course_id);
	...
	public function ajax_list_course()
	{
		...
		foreach ($courses as $course) {
			...
			if ($course->course_img) {
				$row[] = '<img src="'.$course->course_img.'" style="max-height: 100px; max-width: 100px;" />';
			} else {
				$row[] = '';
			}
	...
	public function ajax_list_member()
	{
		...
		foreach ($team as $member) {
			...
			if ($member->member_photo) {
				$row[] = '<img src="'.$member->member_photo.'" style="max-height: 100px; max-width: 100px;" />';
			} else {
				$row[] = '';
			}
